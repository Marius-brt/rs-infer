...
fn logit_and_sigmoid_single_column() {
	let fwd = Fwd { shape: vec![2, 1], data: vec![0.0, 2.0] };
...
fn auto_single_column_is_normalized_0_1() {
	let fwd = Fwd { shape: vec![3, 1], data: vec![-11.0, 0.0, 8.5] };
...
fn softmax_two_column() {
	let fwd = Fwd { shape: vec![1, 2], data: vec![0.0, 0.0] };
...
fn yes_no_uses_last_real_token() {
	...
	let fwd = Fwd { shape: vec![1, 2, 3], data: vec![0.0, 0.0, 10.0, /* pos 1 */ 0.0, 5.0, 0.0] };
