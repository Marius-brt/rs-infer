...
`ortinfer-server` takes a single flag:
...
```bash
ortinfer-server --config configs/config.example.yaml
```
...
```bash
RUST_LOG=info ./target/release/ortinfer-server --config config.yaml   # startup + request summaries
RUST_LOG=debug ./target/release/ortinfer-server --config config.yaml  # + EP/session/file-resolution detail
```
